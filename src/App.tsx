...
/**
 * LOG VOYAGER - PLATINUM EDITION (NO DEMO)
 * - Usunięto funkcje Demo.
 * - Dodano: Obsługa Regex w wyszukiwarce.
 * - Dodano: Panel Ustawień (Rozmiar czcionki, Zawijanie wierszy).
...
const LogLine = ({ content, number, highlight, isBookmarked, onToggleBookmark, id, settings, useRegex }: any) => {
  ...
  const renderContent = () => {
    ...
    if (useRegex) {
      try {
        parts = content.split(new RegExp(`(${highlight})`, 'gi'));
      } catch (e) {
    ...
    return parts.map((part: string, i: number) => {
      ...
      return isMatch
        ? <span key={i} className="bg-[#00f3ff] text-black font-bold px-1 rounded-sm">{part}</span>
